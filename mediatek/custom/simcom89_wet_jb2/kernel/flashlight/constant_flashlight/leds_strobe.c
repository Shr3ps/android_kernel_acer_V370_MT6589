...
int FL_Enable(void)
{
	...
	PK_DBG(" FL_Enable\n");
			...
			{
				mt_set_gpio_mode(GPIO_TORCH_LED_EN,GPIO_FLASH_LED_EN_M_GPIO);
				mt_set_gpio_dir(GPIO_TORCH_LED_EN,GPIO_DIR_OUT);
				mt_set_gpio_out(GPIO_TORCH_LED_EN,GPIO_OUT_ZERO);

				mt_set_gpio_mode(GPIO_FLASH_LED_EN,GPIO_FLASH_LED_EN_M_GPIO);
				mt_set_gpio_dir(GPIO_FLASH_LED_EN,GPIO_DIR_OUT);
				mt_set_gpio_out(GPIO_FLASH_LED_EN,GPIO_OUT_ONE);
			}
			else
			{
				mt_set_gpio_mode(GPIO_FLASH_LED_EN,GPIO_FLASH_LED_EN_M_GPIO);
				...
				mt_set_gpio_mode(GPIO_TORCH_LED_EN,GPIO_FLASH_LED_EN_M_GPIO);
				mt_set_gpio_dir(GPIO_TORCH_LED_EN,GPIO_DIR_OUT);
				mt_set_gpio_out(GPIO_TORCH_LED_EN,GPIO_OUT_ONE);
			}
#elif defined(SIMCOM_FOR1T)
			if(flashlight_mode==1)
			{
							
				mt_set_gpio_mode(GPIO_CAMERA_FLASH_EXT2_PIN,GPIO_CAMERA_FLASH_EXT2_PIN_M_GPIO);
				mt_set_gpio_dir(GPIO_CAMERA_FLASH_EXT2_PIN,GPIO_DIR_OUT);
				mt_set_gpio_out(GPIO_CAMERA_FLASH_EXT2_PIN,GPIO_OUT_ZERO);
				mt_set_gpio_mode(GPIO_CAMERA_FLASH_EXT1_PIN,GPIO_CAMERA_FLASH_EXT1_PIN_M_GPIO);
...
int FL_dim_duty(kal_uint32 duty)
{
	...
	#if defined(SIMCOM_FOR1T)||defined(ACER_C11)
	if(duty<=6)	
	{
		flashlight_mode=0;	
	}
