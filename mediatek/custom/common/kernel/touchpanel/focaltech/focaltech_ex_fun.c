...
#if defined(SIMCOM_C15)
extern unsigned char DISP_GetXinliLCM(void);
static unsigned char *CTPM_FW;
static unsigned char CTPM_FW_XINLI[] = {

	#include "ft_app_xinli.i"
};
static unsigned char CTPM_FW_LAIBAO[] = {

	#include "ft_app_laibao.i"
};
int fw_len = 0;
#else
static unsigned char CTPM_FW[] = {
...
//punk
int fts_dma_i2c_Read(struct i2c_client *client, char *writebuf,
		    int writelen, char *readbuf, int readlen)
{
	int ret;
...
/*write data by i2c*/
int fts_dma_i2c_Write(struct i2c_client *client, u32 writebuf, int writelen)
{
	int ret;
...
#if defined(SIMCOM_C15)
	if (DISP_GetXinliLCM()==0)
	{
		CTPM_FW= CTPM_FW_LAIBAO;
		fw_len=sizeof(CTPM_FW_LAIBAO);
	}
	else
	{
...
#endif
	fts_read_reg(client, FT6x06_REG_FW_VER, &uc_tp_fm_ver);
			  ...
			  u32 dw_lenth)
{
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) 
	{
		...
		delay_qt_ms(upgradeinfo.delay_55+i*5);
	...
	printk("[FTS] Step 3:is_5336_new_bootloader=%d,is_5336_fwsize_30=%d\n",is_5336_new_bootloader,is_5336_fwsize_30);	
...
//	DBG("Step 4:erase app and panel paramenter area\n");
	...
	for (j = 0; j < packet_number; j++) {
		...
		fts_dma_i2c_Write(client,I2CDMABuf_pa , FTS_PACKET_LENGTH + 6);
	...
	if ((dw_lenth) % FTS_PACKET_LENGTH > 0) {
		...
		fts_dma_i2c_Write(client,I2CDMABuf_pa , temp + 6);
		delay_qt_ms(20);
	...
	if(is_5336_new_bootloader == BL_VERSION_LZ4 || is_5336_new_bootloader == BL_VERSION_Z7 )
	{
		for (i = 0; i<6; i++)
		{
			...
			bt_ecc ^= packet_buf[6];
		for (j = 0; j < 7; j++) {
			*(I2CDMABuf_va+j)=packet_buf[j];
		}

		//ft5x0x_i2c_Write(client, packet_buf, 7);
		fts_dma_i2c_Write(client,I2CDMABuf_pa , 7);
		delay_qt_ms(20);
					...
					char *buf)
{
	...
	if (fts_read_reg(client, FT6x06_REG_FW_VER, &fwver) < 0)
...
*/
static DEVICE_ATTR(ftstpfwver, 0664, fts_tpfwver_show,
			fts_tpfwver_store);

/*upgrade from *.i
*example: echo 1 > ftsfwupdate
*/
static DEVICE_ATTR(ftsfwupdate, 0664, fts_fwupdate_show,
			fts_fwupdate_store);
...
*/
static DEVICE_ATTR(ftstprwreg, 0664, fts_tprwreg_show,
			fts_tprwreg_store);
...
*/
static DEVICE_ATTR(ftsfwupgradeapp, 0664, fts_fwupgradeapp_show,
